...
お米を年間20精米トン以上売る事業者には、**帳簿を付けて3年間とっておく義務**が
あります（食糧法第48条。付けていない・嘘を書いた・捨てたは20万円以下の過料）。
その帳簿をこの画面で作ります。

**販売した分は入力不要です。**「注文」に入っているデータから自動で集計されます。
入力が要るのは、注文データでは分からない **仕入・収穫・精米・棚卸** の4つだけです。
...
**「区分の設定」タブ**
1. 事業者名を確認（ふつうは阿部農園のまま）
2. もち米を売っているなら、その商品を「もち」に変える
3. 卸先が届出事業者（JA・米卸・米穀店など）なら「届出事業者の取引先」に追加

**「買受・在庫の入力」タブ**

4. 年度が始まる前の日付（例：**2026/3/31**）で区分「**在庫（実地棚卸）**」を1件入れる
   → これが**期首在庫**になります。倉庫にあった玄米のkgを入れてください

---

#### ② ふだん、こういうことがあったら記録する

すべて「**買受・在庫の入力**」タブの上のフォームです。

| こういうとき | 区分に選ぶもの | 入れる数量 |
|---|---|---|
| よそから米を**仕入れた** | 買受（仕入） | 仕入れたkg（相手方も書く） |
| 自分の田んぼの米を**倉庫に入れた**（収穫） | 自家生産入庫 | 玄米のkg |
| 玄米を**精米した** | とう精（玄米→精米） | 投入した玄米kg＋できた精米kg |
| **棚卸し**して実際の在庫を数えた | 在庫（実地棚卸） | 数えたkg |
| ロス・自家消費などで**減った** | その他増減 | 減った分＋「マイナス」に✓ |

> **とう精は忘れずに。** 売るのは精米、倉庫にあるのは玄米なので、
> これを入れないと精米の在庫がマイナスになっていきます。

---

#### ③ 年度末（3月31日ごろ）にやること

1. 3/31付で「**在庫（実地棚卸）**」を入れる（うるち玄米・うるち精米…と、
   在庫がある区分ごとに1件ずつ）
2. 「**年度の帳簿**」タブで **帳簿CSV** と **帳簿PDF** の両方を保存
3. **3年間**とっておく（PDFは印刷してファイルに綴じてもOK）

---

#### 表の見かた

- **在庫数量** … 棚卸しをしていればその実数、していなければ計算値が出ます
- **差異** … 実際に数えた数と計算が合わない分。ロスや記録漏れの目安
- **判定対象（精米トン）** … これが**20トン以上なら届出が必要**。
  玄米は0.91倍して精米に換算しています

#### 黄色い注意が出たら

| 出るもの | 意味・直しかた |
|---|---|
| 在庫数量がマイナス | とう精の記録が足りない。玄米→精米の記録を足す |
| 複合商品で精米kgが未入力 | 「注文」画面でその注文の精米kgを入れる |
| 期首在庫が未登録 | ①の手順4をやる |

#### 20トン判定から外せるもの

- **無償譲渡**（贈答・サンプル）… 「区分の設定」タブでその注文に✓
- **自家生産米を届出事業者へ出した分**… 卸先を「届出事業者の取引先」に登録

どちらも判定から自動で除かれます。売上や帳簿の販売数量からは消えません。
"""
...
def view_ledger() -> None:
    ...
    st.caption("届出事業者は帳簿を備え、種類別に①買受数量②販売数量③在庫数量を記載して"
               "3年間保存する義務があります。")
    with st.expander("📖 使い方（はじめての方はこちら）"):
        st.markdown(LEDGER_HELP)
