...
ledger_entries = Table(
    ...
    Column("to_location", String(64), default=""),  # 保管場所の移動（kind=move）の移動先
)
...
def init_db() -> None:
    ...
    for col in ("location", "variety", "to_location"):
        if col not in lcols:
            with engine.begin() as c:
...
def _tel_digits(s) -> str:
    return re.sub(r"\D", "", str(s or ""))


def _match_customer_id(conn, name: str, tel: str, address: str):
    """同一人物の既存顧客IDを探す。

    氏名一致を前提に、電話番号の一致を最優先、無ければ住所一致で判定する。
    こうすることで引っ越し（住所変更）でも電話が同じなら同一人物として
    更新でき、別人を取り違えにくい。見つからなければ None。
    """
    name_n = _match_key(name)
    if not name_n:
        return None
    tel_d = _tel_digits(tel)
    addr_n = _match_key(address)
    rows = conn.execute(
        select(customers.c.id, customers.c.name, customers.c.tel, customers.c.address)
    ).mappings().all()
    by_addr = None
    for r in rows:
        if _match_key(r["name"]) != name_n:
            continue
        if tel_d and _tel_digits(r["tel"]) == tel_d:
            return r["id"]                 # 氏名＋電話一致＝最も確実
        if addr_n and _match_key(r["address"]) == addr_n:
            by_addr = r["id"]              # 氏名＋住所一致（電話一致が無いときに採用）
    return by_addr
...
def upsert_customer(data: dict) -> int:
    ...
    同一人物（氏名＋電話／氏名＋住所）が居れば、その既存レコードを最新情報で
    更新する。住所変更でも重複を作らない。なお、空の新値で既存の値（フリガナ・
    会社名など、取込元が持たない項目）を消さないよう、空欄は既存値を保持する。
    """
    with get_engine().begin() as c:
        cid = _match_customer_id(c, data.get("name", ""),
                                 data.get("tel", ""), data.get("address", ""))
        if cid:
            existing = c.execute(
                select(customers).where(customers.c.id == cid)
            ).mappings().first()
            vals = {f: (data.get(f) or existing[f]) for f in _CUST_FIELDS}
...
def add_ledger_entry(data: dict) -> int:
    ...
    for f in ("location", "variety", "to_location"):
        vals[f] = vals[f] or ""
    vals["created_at"] = datetime.now().isoformat()
    with get_engine().begin() as c:
