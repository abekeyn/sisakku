...
# フィールド個別のエラー文（例：「お届け先町・番地が入力されていません。」）。
# 画面上部の定型バナー（登録内容を訂正の上…）より、原因の特定に直接役立つので最優先で拾う。
_SPECIFIC_KEYS = ("入力されていません", "正しくありません", "一致しません",
                  "超えています", "登録されていません", "利用いただけません",
                  "選択してください", "設定されていません")
_GENERIC_BANNERS = (
    "登録内容を訂正の上、再度実行してください", "修正が必要です",
    # 個別行のエラーとは無関係に常時表示される定型注意書き（実際は誤検知の原因になった）
    "クロネコゆうメールとクロネコゆうパケットでは、営業所止め置きサービスをご利用いただけません",
)
...
def _read_b2_fix_reason(page) -> str:
    ...
    優先順位：①「○○が入力されていません」等のフィールド個別メッセージ（全文検索）
             ②赤文字の短いテキスト（フォールバック）
    ①のほうが「登録内容を訂正の上、再度実行してください」という定型バナーより
    はるかに原因特定に役立つため、見つかればそちらを優先する。
    """
    red_js = """() => {
        const out=[];
    ...
    try:
        ...
        for fr in target.frames:
            try:
                body_text = fr.evaluate("() => document.body ? document.body.innerText : ''")
            except Exception:  # noqa: BLE001
                continue
            for line in (body_text or "").splitlines():
                line = line.strip()
                if not line or line in specific:
                    continue
                if any(b in line for b in _GENERIC_BANNERS):
                    continue
                if any(k in line for k in _SPECIFIC_KEYS):
                    specific.append(line)
        if specific:
            return " / ".join(specific[:5])[:400]
